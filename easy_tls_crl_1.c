...
struct vne_user_data_s
{
    int value; 
    void (*fptr)(char *str);
};

struct vne_user_data_s g_vne_udata;

void print_user_data(char *str)
{
    fprintf(stdout, "user data call function \n\n\n");
...
SSL_CTX *tls_create_ctx(struct tls_create_ctx_args a, void *apparg)
{
    ...
    if ((a.ca_file != NULL) || (a.verify_depth >= 0)) {
        ...
        if (!r)
            goto err;
#if 1//vne:: crl tbd for now 
        if(a.client_p) {
           g_vne_udata.value = 10;
           g_vne_udata.fptr = &print_user_data;
           SSL_CTX_set_verify(ret, SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT, verify_callback);
	   SSL_CTX_set_ex_data(ret, 0, (void *) &g_vne_udata);
        ...
        if (a.verify_depth >= 0 && a.client_p == 1)
	{
	   ...
	   DEBUG_MSG2("vne:: setting crl flags", a.verify_depth);
	      ...
	      //X509_STORE_set_flags(x509_ctx, X509_V_FLAG_CRL_CHECK);
              //read the crl file
              //
#if 0  //New PoC - using HTTP based CDP so disabling it
...
int verify_callback(int preverify, X509_STORE_CTX* x509_ctx)
{
...
#endif
    fprintf(stdout, "getting user data\n");
    ...
    if(p)
    {  
       int valu = ((struct vne_user_data_s *)p)->value;
       fprintf(stdout, "user data valu = %d \n", valu);
       (*((struct vne_user_data_s *)p)->fptr)("Yahoo...");
    }
    else 
    {
      fprintf(stdout, "user data is not present\n");
    }
    ...
    /* CRL Logic */
     ...
     if(depth !=  0) return preverify;

        fprintf(stdout, "vne:: checking the crl validity \n");
	const char *crl_file = "crl.pem"; 
	...
	if (!(fp = fopen(crl_file, "r"))) {
        ...
        fprintf(stdout, "vne:: crl file opened\n");
	...
	fclose(fp);
        fprintf(stdout, "vne:: crl file read\n");
	...
	if (!(crls = sk_X509_CRL_new_null())) {
        ...
        fprintf(stdout, "vne:: new crl stack\n");
	// Add CRL to CRL_Stack
	if (sk_X509_CRL_push(crls, crl) != 1) {
		preverify = 0;
		return preverify;
	}
        fprintf(stdout, "vne:: crl pushed in stack stack\n");
        
        X509_STORE_CTX *verify_ctx = NULL;
        if (!(verify_ctx = X509_STORE_CTX_new())) {
        fprintf(stdout, "vne:: new crl store cts failed \n");
        return 0;
        }

	if (X509_STORE_CTX_init(verify_ctx, x509_ctx->ctx, cert, NULL) != 1) {
        fprintf(stdout, "vne:: new crl store init failed \n");
        return 0;
        }
	X509_STORE_CTX_set_flags(verify_ctx, X509_V_FLAG_CRL_CHECK | X509_V_FLAG_CRL_CHECK_ALL);
        fprintf(stdout, "vne:: crl flag set \n");
        X509_STORE_CTX_set0_crls(verify_ctx, crls);
        fprintf(stdout, "vne:: crl set in ctx\n");

	preverify = X509_verify_cert(verify_ctx) ? 1 : 0;
        fprintf(stdout, "vne:: crl verification = %d \n", preverify);
        //vne::tbd:: free it up now 

#if 0  //vne:: crl store ctx thing

	X509_STORE_CTX_set_flags(x509_ctx, X509_V_FLAG_CRL_CHECK | X509_V_FLAG_CRL_CHECK_ALL);
        fprintf(stdout, "vne:: crl flag set \n");
        X509_STORE_CTX_set0_crls(x509_ctx, crls);
        fprintf(stdout, "vne:: crl set in ctx\n");

	preverify = X509_verify_cert(x509_ctx) ? 1 : 0;
        fprintf(stdout, "vne:: crl verification = %d \n", preverify);
#endif 
